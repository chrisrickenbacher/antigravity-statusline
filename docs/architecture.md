...
2. **Billing Daemon (`statusline-daemon`)**: A background service (configured via systemd or launchd) that runs periodically, queries GCP Monitoring for active Vertex AI usage metrics, fetches model pricing rates, and atomically caches the data as JSON files.
